...
const KIND = 'voice' as const;
const DEFAULT_COST = Number(process.env.VOICE_COST || 5);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const cost = calculateBillingCost({
      billingMode: systemConfig.audioProvider?.voiceBillingMode,
      billingPrice: systemConfig.audioProvider?.voiceBillingPrice,
      billingUnit: systemConfig.audioProvider?.voiceBillingUnit,
      legacyCost: systemConfig.audioProvider?.voiceCost || DEFAULT_COST,
    });
